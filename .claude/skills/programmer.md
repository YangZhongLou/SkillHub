...
- **CRITICAL: No destructive data operations.** Never delete repositories, drop databases/tables, run `rm -rf`, force push to main, or execute any irreversible command without explicit user confirmation. When in doubt, ask first.
...
- **Error handling:** Fail fast and loud. Don't silently swallow errors. Handle errors at the level where you can actually do something about them. Don't add error handling for scenarios that can't happen.
